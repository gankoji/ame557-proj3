...
def lambert(r1vec, r2vec, tf, m, muC):
    ...
    dth = acos( max(-1, min(1, (np.dot(r1vec,r2vec)/mr2vec)) ))
    ...
    crossprd = np.cross(r1vec,r2vec)               # non-dimensional normal vector
    mcr       = np.linalg.norm(crossprd)           # magnitude thereof
    ...
    y12 = np.zeros((2,))
    y12[0]  = (aa[0]*sqrt(aa[0])
            *((aalfa - sin(aalfa))
              - (bbeta-sin(bbeta))
              + 2*pi*m))

    y12[1]  = (aa[1]*sqrt(aa[1])
            *((aalfa - sin(aalfa))
              - (bbeta-sin(bbeta))
              + 2*pi*m))
